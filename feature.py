...
class FeatureExtraction:
    ...
    def AnchorURL(self):
        try:
            ...
            for a in self.soup.find_all('a', href=True):
                if "#" in a['href'] or "javascript" in a['href'].lower() or "mailto" in a['href'].lower() or not (url_for in a['href'] or self.domain in a['href']):
                    unsafe = unsafe + 1
                i = i + 1
    ...
    def WebsiteTraffic(self):
        try:
            rank = BeautifulSoup(urllib.request.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + urlsafe_b64decode).read(), "xml").find("REACH")['RANK']
            if (int(rank) < 100000):
                return 1
            return 0
        except :
